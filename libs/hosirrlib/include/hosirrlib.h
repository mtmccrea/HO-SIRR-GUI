...
#define HOSIRR_MAX_SH_ORDER (7)
    ...
    typedef enum
    {
        ...
        //    */ FDN_BANDS_SPLIT,        /**< FDN is split into bands */ FDN_EDC_DONE /**< EDCs are
        //    calculated for each beam/band of the FDN channels */
...
#define HOSIRR_PROGRESSTEXT_CHAR_LENGTH (256)
    ...
    int hosirrlib_setRIR(void* const hHS, const float** H, int numChannels, int numSamples,
        int sampleRate,
    ...
    void hosirrlib_setDirectOnsetIndices(void* const hHS, float* const brdbndBuf,
        float*** const bndBuf, const float thresh_dB, ANALYSIS_STAGE thisStage);
    void hosirrlib_setDiffuseOnsetIndex_shd(void* const hHS,
    ...
    void hosirrlib_setDiffuseOnsetIndex_mono(void* const hHS, float* const monoBuf,
        ...
        const int hopSize, const int startSamp, ANALYSIS_STAGE thisStage);
    ...
    void hosirrlib_calcT60_beams(void* const hHS, float*** const edcBuf, float** const t60Buf,
        const int nBand, const int nDir, const int nSamp, const float startDb, const float spanDb,
    ...
    void hosirrlib_calcT60_omni(void* const hHS, float** const edcBuf_omn, float* const t60Buf,
        const int nBand, const int nSamp, const float startDb, const float spanDb,
        const int beginIdx, ANALYSIS_STAGE thisStage);
    ...
    void hosirrlib_findDecayBounds(float* const edcBuf, const int beginIdx, const int bufLength,
        const float start_db, const float span_db, int* const st_end_meas);
    ...
    void checkProperProcessingOrder(
        void* const hHS, ANALYSIS_STAGE currentStage, const char* funcName);
