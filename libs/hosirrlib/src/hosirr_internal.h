...
static void hosirrlib_FIRCoeffs
(
...
)
{
    ...
    h_len = order + 1;
    ft1 = fc1/fs; // corrected, was ft1 = fc1/(fs*2.0f); see §8.1.1, J. G. Proakis and D. G. Manolakis, Digital Signal Processing: Principles, Algorithms, and Applications.
...
)
{
    ...
    hosirrlib_FIRCoeffs(FIR_FILTER_LPF, order, fc[0], 0.0f, sampleRate, windowType, scalingFLAG, filterbank);
    hosirrlib_FIRCoeffs(FIR_FILTER_HPF, order, fc[nCutoffFreq-1], 0.0f, sampleRate, windowType, scalingFLAG, &filterbank[(nFilt-1)*(order+1)]);
    
    /* the inbetween bands are then band-pass filters: */
    if(nCutoffFreq>1){
        for(k=1; k<nFilt-1; k++)
            hosirrlib_FIRCoeffs(FIR_FILTER_BPF, order, fc[k-1], fc[k], sampleRate, windowType, scalingFLAG, &filterbank[k*(order+1)]);
    }
}
